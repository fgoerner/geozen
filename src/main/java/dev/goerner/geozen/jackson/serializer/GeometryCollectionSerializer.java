        ...
        for (Geometry geometry : value.getGeometries()) {
            switch (geometry) {
                ...
                case LineString lineString ->
                        ctxt.findValueSerializer(LineString.class).serialize(lineString, gen, ctxt);
                case Polygon polygon -> ctxt.findValueSerializer(Polygon.class).serialize(polygon, gen, ctxt);
                case MultiPoint multiPoint ->
                        ctxt.findValueSerializer(MultiPoint.class).serialize(multiPoint, gen, ctxt);
                ...
                default ->
                        throw new IllegalArgumentException("Invalid Geometry type: " + geometry.getClass().getSimpleName());
            }
        }
        gen.writeEndArray();
