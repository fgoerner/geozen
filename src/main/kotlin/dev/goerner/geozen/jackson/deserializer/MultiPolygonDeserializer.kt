...
class MultiPolygonDeserializer : AbstractGeometryDeserializer<MultiPolygon>() {
    ...
    override fun deserialize(p: JsonParser, ctxt: DeserializationContext): MultiPolygon {
        ...
        require(coordinatesNode != null && coordinatesNode.isArray) {
            "Invalid or missing 'coordinates' field for MultiPolygon geometry."
        }
        val coordinates =
            (coordinatesNode as Iterable<JsonNode>).map { polygonNode ->
                require(polygonNode.isArray) {
                    "Invalid Polygon in 'coordinates' field for MultiPolygon geometry."
                }
                (polygonNode as Iterable<JsonNode>).map { ringNode ->
                    require(ringNode.isArray) {
                        "Invalid linear ring in 'coordinates' field for MultiPolygon geometry."
                    }
