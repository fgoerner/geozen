...
/**
 * A [MultiPolygon] is a [Geometry] that represents a collection of
 * [Polygons][dev.goerner.geozen.model.simple_geometry.Polygon] in space. It is defined by a list of
 * [Polygons][dev.goerner.geozen.model.simple_geometry.Polygon] and a [CoordinateReferenceSystem].
 *
 * @param coordinates The list of polygons (each represented as a list of rings, where each ring is
 *   a list of positions). The outer list may be empty. Each polygon must have at least 1 ring
 *   (exterior ring). Each ring must contain at least 4 positions and be closed (first position
 *   equals last position).
 * @param coordinateReferenceSystem The coordinate reference system, defaults to WGS_84
 * @throws IllegalArgumentException if any polygon has no rings, if any ring has fewer than 4
 *   positions, or if any ring is not closed (first position != last position)
 */
    ...
    init {
        coordinates.forEachIndexed { polygonIndex, polygon ->
            ...
            polygon.forEachIndexed { ringIndex, ring ->
                require(ring.size >= 4) {
                    val ringType =
                        if (ringIndex == 0) "exterior ring" else "interior ring at index $ringIndex"
