...
object PreciseDistanceCalculator : AbstractDistanceCalculator() {
     ...
     *
     * This method utilizes the GeographicLib library by calling `Geodesic.WGS84.Inverse` with the
     * latitude and longitude of the provided positions, employing the `GeodesicMask.DISTANCE` mask.
     * The resulting `GeodesicData` object's `s12` field contains the computed distance in meters.
     *
            ...
            val dist =
                if (azDiffA > 90) {
                    ...
                    if (azDiffB > 90) {
                        ...
                        val projectionDistance =
                            distanceFromA * kotlin.math.cos(Math.toRadians(azDiffA))
     ...
     * 2. If the result exceeds 180, subtract it from 360 to fold it back into [0, 180]. (e.g. a
     *    200° difference equals the same angular separation as 160°, just measured the other way
     *    around the circle.)
     *
     * The resulting value represents the smallest angular separation between the two azimuths,
     * which is then compared against 90° to decide whether the closest point on a geodesic segment
     * lies at an endpoint or in the interior.
     *
